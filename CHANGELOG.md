...
### Fixed
- **Fleet rows no longer show a redundant second status glyph beside the selection marker.** `⏺`/`◯` alone indicates selection; running/queued/completed/error state now uses the task description's semantic color without adding another circle/check/cross column. The widget turn counter also keeps a space after `↻`, avoiding terminals visually joining the glyph to its first digit.
