...
### Added
- **True-tree FleetView across nested Agent managers.** FleetView now projects trusted lineage into a preorder tree with ancestor connectors, explicit missing-parent orphans, stable launch-time/ID sibling order, default expansion, and `←`/`→` collapse/expand/parent/child navigation. A minimal process-global `Symbol.for()` owner registry asks every manager for live records on each read (without copying records or becoming a second fact source), isolates broken providers, and routes open, steer, and stop to the correct owner, so a root session can see dynamically loaded grandchildren. Selection follows stable identity through reorder/tree changes, the five-visible-agent window ignores collapsed descendants and keeps selection visible, finished/viewing semantics are preserved, and all lines remain width-clamped. This changes FleetView only; `/agents` remains a flat management roster.
- **Color-aware, structured Agent conversation viewer.** The opened-Agent overlay now groups history into numbered user turns and preserves assistant execution order across Markdown text, folded thinking, tool calls, and paired results. Tool blocks use mechanical argument previews, semantic running/success/error styling, bounded output policies, and a keybinding-aware detail toggle (`Ctrl+O` by default); provider errors, images, custom entries, branch/compaction summaries, trusted lineage, and live activity all have explicit presentations. Rendering uses only Pi's public Markdown/theme/TUI APIs, clamps every line safely, and bounds pathological histories and outputs. The separate `/agents` roster is unchanged.
- **Authorized cascading Agent stops.** The new closed-schema `stop_agent` tool lets an Agent stop only its trusted-lineage direct child and always stops that child's full active current-process subtree, deepest-first. A `Symbol.for()` runtime tree routes control across dynamically loaded extension managers while retaining completed-parent skeletons around live descendants, atomically marks cascades as stopping before owner callbacks, blocks late spawn/resume races, and keeps unknown, cross-tree, cross-project, sibling, ancestor, and grandchild targets non-disclosing. Maximum-level Agents do not receive the tool, repeated calls are idempotent, and the existing caller-less cross-extension RPC stop remains a single-manager compatibility operation.
- **Low-token Agent observation tools.** `inspect_agent` now returns compact status facts and can attach bounded `tail` or cursor-based `after` entry metadata without exposing prompts, results, errors, message bodies, summaries, previews, or transcript sources. Stable opaque refs use a 96-bit SHA-256 prefix over the complete session-ID/entry-ID pair without exposing either ID; `read_agent_entry` then retrieves only that canonical JSON entry through UTF-8-safe byte ranges. Live reads use the Agent session manager, while durable reads use bounded Buffer-based forward/reverse JSONL scans with fatal UTF-8 validation and an 8 MiB line limit, and never migrate, rewrite, or consult `.output` files. Same-project authorization follows existing Agent lookup governance, stale/cross-session cursors fail explicitly, malformed/missing/permission failures are structured, and memory history is unavailable after process cleanup. Both tools follow recursive supervision gating and remain hidden at the maximum tree level; `get_subagent_result` compatibility is unchanged.
- **Asynchronous direct-relation Agent mailbox.** The new `mailbox` tool lets a parent and its direct children exchange persisted, at-least-once messages without steering or changing conversation history. Sender identity comes only from trusted session lineage; sibling, ancestor/descendant, cross-root, and unknown targets share a non-disclosing rejection. Durable mail shares the PID-locked atomic Agent session index and survives restart, while memory-Agent mail stays in a guarded process-wide registry shared across extension activations. Receive peeks until an idempotent acknowledgement, acknowledged messages remain stored, and maximum-level Agents retain mailbox access even though Agent/result/steer remain hidden.
...
- **Call-level Agent session governance.** Spawn operations accept `session_persistence: "durable" | "memory"` with call > frontmatter > durable-default precedence. Memory sessions remain usable only in the current Pi process and write no child Session JSONL, durable Agent index, completion record, or `.output` transcript. UI cards, Fleet rows, conversation details, and `/agents` now expose persistence; durable details include live file size/path plus creation and last-resume timestamps.

### Changed
- **FleetView is now the default Agent status projection with root-aware Widget deduplication.** While FleetView is enabled, it owns the current root and the above-editor Widget omits only that root; running Agents from switched-away roots remain visible in the Widget. While Fleet is suspended or unbound—and whenever it is disabled—the Widget covers every record allowed by the user's stored `all`/`background`/`off` choice, avoiding both duplicate rows and session-switch visibility gaps.
- **Agent calls now inherit the main agent's model and effort when omitted.** `model`, `thinking`, and the UI-only `description` are optional; descriptions derive from the task prompt, while new and scheduled runs require `subagent_type` and resumed runs reuse their original execution identity. Explicit model overrides remain strict exact `provider/modelId` values, scheduled jobs freeze the resolved identity at creation, and the built-in `Explore` agent no longer pins Haiku.
- **The Agent input contract is now a provider-compatible discriminated operation schema.** Every call supplies exactly one nested `operation`: `spawn` requires `prompt` + `subagent_type`, `resume` requires `agent_id` + `prompt`, and enabled scheduling adds `schedule` requiring `schedule` + `prompt` + `subagent_type`. Invalid cross-operation fields are rejected by nested `additionalProperties: false`; resume no longer advertises meaningless identity or isolation overrides. Legacy flat arguments remain normalized only for direct/RPC compatibility.

### Fixed
- **FleetView uses an `AGENTS` section heading instead of a blank spacer below its navigation hint.** The heading uses the active accent theme, preserving separation while reclaiming the otherwise meaningless empty row.
- **FleetView can be dismissed and reliably restored without stealing editor history.** The already-visible tree now hides when active selection is on `main` and `↑` is pressed; inactive `↑` passes through, and `Esc` exits navigation without hiding it. Dismissal retains the sole widget/TUI focus context and refresh timer while rendering no lines, so empty-prompt `↓`/`←` restores only with explicit editor focus; unknown focus, selectors, and menus fail open. Truly empty or disabled trees unregister, and an empty transition resets dismissal so the next Agent batch appears automatically.
- **Active descendants retain their complete real ancestor chain.** Eligibility now starts from running, queued, viewed, or lingering records and recursively includes live-record ancestors before trusted tree validation. Thus an old completed parent stays selectable while its grandchild runs, then the whole branch disappears after the descendant's linger; genuinely missing or invalid lineage remains orphaned or rejected rather than fabricated.
- **Fleet rows now consolidate all live Agent status into one line.** `⏺`/`◯` is the sole selection marker; no second status glyph or activity/details row is emitted. The description and non-generic type are followed by dimmed short model ID, thinking level, persistence, `↻ turns≤limit`, and nonzero tool count, while elapsed time and compact `↓ 16.9k` usage stay right-aligned. Sessionless queued records appear immediately with metadata and report that no session is available on `Enter`. Narrow layouts truncate left-tail metadata first, including exact right-block-width boundaries without a forced gap or lost final character; every line remains clamped, and transient activity text such as `running command…` is omitted. The shared turn counter also keeps a space after `↻`, avoiding terminals visually joining the glyph to its first digit.
- **Conversation turns no longer repeat the `ASSISTANT` lane heading.** A turn containing multiple assistant messages (for example text, thinking/tool/result, then final text) now prints the lane title once and appends all later parts and provider errors in their existing projected order; user and custom/meta ordering is unchanged, including anomalous repeated user items.
- **Terminal Agent errors now carry a directly readable `error_ref`.** Final `error` records scan the live session first or bounded durable JSONL in reverse for the newest real provider/tool error entry, persist its existing opaque digest ref, and attach it independently to individual/group notification XML and renderer details. Parents can pass the ref straight to `read_agent_entry` without an inspection-tail round trip; stopped, aborted, steered, token-length failures without a canonical error entry, and recoverable tool errors on successful runs do not fabricate refs. Agent stopping was also hardened so queued/running aborts persist stop reasons and partial output, resumed runs use a fresh effective abort controller, and late completion/error races cannot overwrite `stopped`.
...
- **Durable resume failures are now precise and non-destructive.** Explicit resume distinguishes unknown IDs, incomplete lookup due to unreadable/corrupt parent indexes, missing session references, missing files, permission failures, non-file paths, and malformed JSONL. Corrupt index files are never silently overwritten; startup warns and leaves them untouched.
## [0.14.2] - 2026-07-17
